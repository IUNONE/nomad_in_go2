...
/*
    to Get request messages corresponding to high-level motion commands according to the API_ID
*/
...
void SportClient::TrajectoryFollow(unitree_api::msg::Request &req, std::vector<PathPoint> &path)
{
    ...
    req.header.identity.api_id = ROBOT_SPORT_API_ID_TRAJECTORYFOLLOW;   // use api
    for (size_t i = 0; i < 30; i++) // should be 30 here. error when using path.size()
    {
